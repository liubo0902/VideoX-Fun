...
def decode_images(pixel_latents: torch.Tensor, vae: torch.nn.Module, weight_dtype):
    latents_mean = (
        torch.tensor(vae.config.latents_mean).view(1, vae.config.z_dim, 1, 1, 1)
    ).to(vae.device)
    latents_std = 1.0 / torch.tensor(vae.config.latents_std).view(
    ...
    pixel_latents = pixel_latents / latents_std + latents_mean
    pixel = vae.decode(pixel_latents.to(vae.dtype), return_dict=False)[0].squeeze()
    pixel = (pixel * 0.5 + 0.5).clamp(0, 1)
    pixel = (pixel * 255).to(torch.uint8).permute(1, 2, 0).cpu().numpy()
    img = Image.fromarray(pixel)
    return img


def log_validation(qwen_transformer, qwen_cn_transformer, args, accelerator, weight_dtype, step, is_final_validation=False):
        ...
        for _ in range(args.num_validation_images):
            with autocast_ctx:
                image = pipeline(
                    ...
                    guidance_scale=3.5, # args.guidance_scale,
                    generator=generator,
...
def prepare_train_dataset(dataset, accelerator, args):
    ...
    def preprocess_train(examples):
        ...
        images = [image_transforms(resize_transform(image)) for image in images]

        conditioning_images = [
            (image.convert("RGB") if not isinstance(image, str) else Image.open(image).convert("RGB"))
            for image in examples[args.conditioning_image_column]
        ]
        conditioning_images = [image_transforms(resize_transform(image)) for image in conditioning_images]
        ...
        if args.proportion_empty_prompts and random.random() < args.proportion_empty_prompts:
            ...
            for (img_idx, img_path) in enumerate(examples[args.conditioning_image_column]):
                captions[img_idx] = ""
...
def main(args):
    ...
    qwen_cn_transformer = QwenImageControlNetModel.from_pretrained(args.cn_pretrained_model_name_or_path, torch_dtype=weight_dtype)
    ...
    if args.gradient_checkpointing:
        qwen_cn_transformer.enable_gradient_checkpointing()
    ...
    optimizer = optimizer_class(
        qwen_cn_transformer.parameters(),
    ...
    qwen_cn_transformer, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        qwen_cn_transformer, optimizer, train_dataloader, lr_scheduler
    )
    ...
    image_logs = None
    if not args.offload:
        qwen_transformer = qwen_transformer.cuda()
    for epoch in range(first_epoch, args.num_train_epochs):
        qwen_cn_transformer.train()
        qwen_transformer.train()
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate([qwen_cn_transformer, qwen_transformer]):
                ...
                noise = torch.randn(pixel_latents.shape, device=accelerator.device, dtype=weight_dtype)
                # timestep_id = torch.randint(0, noise_scheduler_copy.num_train_timesteps, (1,))
                # timestep = noise_scheduler_copy.timesteps[timestep_id].to(dtype=weight_dtype, device=accelerator.device)
                
                # noisy_model_input = noise_scheduler_copy.add_noise(pixel_latents, noise, timestep)
                # target = noise_scheduler_copy.training_target(noisy_model_input, noise, timestep)
                ...
                timestep = noise_scheduler_copy.timesteps[indices].to(device=pixel_latents.device)

                # Add noise according to flow matching.
                sigmas = get_sigmas(timestep, n_dim=pixel_latents.ndim, dtype=pixel_latents.dtype)
                noisy_model_input = (1.0 - sigmas) * pixel_latents + sigmas * noise
                target = noise - pixel_latents
                # Concatenate across channels.

                # pack the latents.
                height, width = noisy_model_input.shape[3], noisy_model_input.shape[4]
                img_shapes = [(1, height // 2, width // 2)]
                num_channels_latents = qwen_transformer.config.in_channels // 4
                noisy_model_input = QwenImagePipeline._pack_latents(
                    noisy_model_input,
                    batch_size=bsz,
                ...
                control_latents = QwenImagePipeline._pack_latents(
                    control_latents,
                    batch_size=bsz,
                    num_channels_latents=num_channels_latents,
                    height=control_latents.shape[3],
                    width=control_latents.shape[4],
                )
                target = QwenImagePipeline._pack_latents(
                    target,
                    batch_size=bsz,
                    num_channels_latents=num_channels_latents,
                    height=height,
                ...
                if accelerator.sync_gradients:
                    params_to_clip = qwen_cn_transformer.parameters()
            ...
            if accelerator.sync_gradients:
                ...
                if True: # accelerator.distributed_type == DistributedType.DEEPSPEED or accelerator.is_main_process:
                    ...
                    if args.validation_prompt is not None and global_step % args.validation_steps == 0:
                        image_logs = log_validation(
                            qwen_transformer=qwen_transformer,
                            qwen_cn_transformer=qwen_cn_transformer,
                            args=args,
                            accelerator=accelerator,
    ...
    if accelerator.is_main_process:
        qwen_cn_transformer = unwrap_model(qwen_cn_transformer)
        if args.upcast_before_saving:
            qwen_cn_transformer.to(torch.float32)
        qwen_cn_transformer.save_pretrained(args.output_dir)
